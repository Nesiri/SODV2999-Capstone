    ...
    <div className="relative overflow-hidden rounded-3xl border border-slate-200 !bg-white shadow-[0_30px_60px_-40px_rgba(2,6,23,0.55)]">
      ...
      <div className="relative grid gap-8 p-8 md:grid-cols-12 md:p-10">
        <div className="md:col-span-7">
          ...
          <h1 className="mt-4 text-3xl font-normal tracking-tight !text-[#5f2d85] md:text-5xl">
            {page.title}
          </h1>
          ...
          <div className="mt-6 flex flex-wrap items-center gap-3">
            <button
              onClick={onBack}
              className={cx(
                'inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-normal text-white shadow-sm transition hover:opacity-95',
                'bg-gradient-to-r',
                `from-${from}`,
                `to-${to}`
              )}
            ...
            <a
              href="#start"
              className="inline-flex items-center gap-2 rounded-xl border border-slate-200 !bg-white/80 px-4 py-2 text-sm font-normal !text-[#5f2d85] shadow-sm transition hover:!bg-white"
            >
              Start here ↓
            </a>
          </div>
              ...
              <div className="grid h-full w-full place-items-center bg-gradient-to-br from-slate-50 to-slate-100 p-8">
                <div className="text-center">
                  ...
                  <p className="mt-1 text-xs text-slate-500">
                    Set{' '}
                    <code className="rounded !bg-white px-1">heroImage</code> in
                    patternData.
                  </p>
                </div>
              </div>
    ...
    <div className="scroll-mt-24" id={index === 0 ? 'start' : undefined}>
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="!text-xl font-normal !text-[#5f2d85] md:text-2xl">
              {heading}
            </h2>
            <div
              className={cx(
            ...
            <div key={r.title}>
              <div className="grid gap-6 md:grid-cols-[240px_1fr] md:items-start">
                <div className="flex flex-col items-center">
                  ...
                  {r.href ? (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="mt-4 max-w-[220px] text-center text-base font-normal !text-[#5f2d85] hover:underline"
                    >
                      {r.title}
                    </a>
                  ) : (
                    <p className="mt-4 max-w-[220px] text-center text-base font-normal !text-[#5f2d85]">
                      {r.title}
                    </p>
                  )}
                </div>
                ...
                <div className="md:pt-2">
                  <p className="leading-7 !text-[#5f2d85]/90">
                    {r.description}
                  </p>
                  {r.moreText ? (
                    <p className="mt-4 leading-7 !text-[#5f2d85]/90">
                      {r.moreText}
                    </p>
                  ) : null}
                </div>
              ...
              <Card className="overflow-hidden transition group-hover:shadow-[0_25px_60px_-40px_rgba(2,6,23,0.65)]">
                ...
                <div className="p-5">
                  <div className="flex items-start justify-between gap-3">
                    <h3 className="text-base font-normal !text-[#5f2d85]">
                      {r.icon ? `${r.icon} ` : ''}
                      {r.title}
                    </h3>
                  ...
                  <div className="mt-4 flex items-center justify-between">
                    ...
                    <span
                      className={cx(
                        'bg-clip-text text-sm font-normal text-transparent',
                        'bg-gradient-to-r',
                        `from-${from}`,
                        `to-${to}`
                      )}
      ...
      <div className="mx-auto max-w-6xl px-6 py-10">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <a
              href="#extras"
              className="rounded-xl border border-slate-200 !bg-white/80 px-3 py-2 text-sm font-normal !text-[#5f2d85] shadow-sm transition hover:!bg-white"
            >
              Helpful extras
            </a>
            <a
        ...
        <div className="mt-10 grid gap-10">
              ...
              <Card className="p-6 md:p-10">
                <div className="space-y-14">
                  <section className="grid items-start gap-10 md:grid-cols-2">
                    <div>
                      <h2 className="mb-3 text-3xl font-normal !text-[#5f2d85]">
          ...
          <div id="more" className="scroll-mt-24">
            <div className="flex items-center justify-between">
              <h3 className="!text-xl font-normal !text-[#5f2d85] md:text-2xl">
                Explore more
              </h3>
              <Link
                to="/pattern"
                ...
                <Link key={key} to={`/pattern/${key}`} className="group block">
                  <Card className="p-6 transition group-hover:shadow-[0_25px_60px_-40px_rgba(2,6,23,0.65)]">
                    <div className="flex items-start justify-between gap-4">
                      ...
                      <span
                        className={cx(
                          'rounded-full px-3 py-1 text-xs font-normal text-white',
                          'bg-gradient-to-r',
                          `from-${from}`,
                          `to-${to}`
                        )}
                    ...
                    <h4 className="mt-4 text-base font-normal !text-[#5f2d85]">
                      {value.title}
                    </h4>
                    <p className="mt-2 text-sm leading-relaxed text-slate-600">
                      {value.description}
          ...
          <div className="mt-2">
            <Card className="p-6 md:p-7">
              <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div>
                  <h4 className="text-base font-normal !text-[#5f2d85]">
                    Quick reminder
                  </h4>
                ...
                <a
                  href="#start"
                  className={cx(
                    'inline-flex items-center justify-center rounded-xl px-4 py-2 text-sm font-normal text-white shadow-sm transition hover:opacity-95',
                    'bg-gradient-to-r',
                    `from-${from}`,
                    `to-${to}`
                  )}
