...
const SiteMapPage = () => {
  ...
  const resourceLinks = [
    "Canadian Crisis Centres", "USA crisis Centres", "International Crisis Centres",
    "Pattern Interrupts", "Coping with Suicidal Thoughts", "Search for a Professional",
    "E Councelling", "Self Management", "How Can I Help Someone", "Warning Signs",
    "Depression", "Myths & Facts", "Addiction", "Anxiety", "Post Traumatic Stress Disorder",
    "Grief", "Teens & Youth", "Parents", "College Students", "LGBTQ2S",
    "Indigenous Issues", "Seniors", "Men", "Veterans & Military", "Refugees",
    "Attempt Survivors", "Survivors of Suicide Loss", "Bereaved Support",
    "After a Suicide Loss", "Educators", "Health Professionals", "First Responders",
    "Care Givers", "Media Professionals", "Workplace Employers", "Resource Organizations",
    "Pattern Interrupts"
  ];

  const patternInterruptsMainLinks = [
    "Acts of Kindness", "Comedy", "Coping Mechanisms", "Emotional Support Videos",
    "Helpful Movies", "Image Gallery", "Improve Your Mood", "Inspiring Videos",
    "Interactive Mind", "Life Changing Books"
  ];

  const mentalHealthAppLinks = [
    { name: "Suicide Prevention", path: "/mental-health-apps/suicide-prevention" },
    { name: "Mood Tracking", path: "/mental-health-apps/mood-tracking" },
    { name: "Depression & Anxiety", path: "/mental-health-apps/depression-anxiety" },
    { name: "Sleep", path: "/mental-health-apps/sleep" },
    { name: "Sleep/Relax/Meditate", path: "/mental-health-apps/sleep-relax-meditate" }
  ];

  const patternInterruptsMoreLinks = [
    "Movies of Inspiration", "Self Management", "Uplifting Music Videos",
    "Transform Negative Thoughts", "Words of Wisdom"
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12 lg:py-16">
        ...
        <ul className="list-none p-0 mb-12 md:mb-16">
          {mainLinks.map((link, idx) => (
            <li key={idx} className="mb-2 md:mb-3 cursor-pointer transition-all duration-300 font-medium">
              <Link 
                to={link.path} 
            ...
            <li key={idx} className="mb-2 md:mb-3 cursor-pointer transition-all duration-300 font-medium">
              {link.name === "Canada" ? (
                <>
                  <Link 
                    to={link.path} 
                    style={{...gradientStyle, fontSize: fontSizes.mainLink, display: 'block', marginBottom: '0.5rem'}}
                  >
                    {link.name}
                  </Link>
                  <ul className="list-none pl-4 md:pl-6 mt-2 mb-4">
                    {crisisProvinceLinks.map((province, pIdx) => (
                      <li key={pIdx} className="mb-1.5 md:mb-2 cursor-pointer transition-all duration-300">
                        <Link 
                          to={province.path} 
        ...
        <ul className="list-none p-0 mb-12 md:mb-16">
          ...
          <ul className="list-none pl-4 md:pl-6 mt-2 mb-4">
            {mentalHealthAppLinks.map((app, idx) => (
              <li key={idx} className="mb-1.5 md:mb-2 cursor-pointer transition-all duration-300">
                <Link 
                  to={app.path} 
                  style={{...gradientStyle, fontSize: fontSizes.subLink}}
                >
